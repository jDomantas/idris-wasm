...
mutual
    ...
    data Instr : CodeCtx -> ResultType -> Type where
        ...
        -- we have overaligned accesses we can just decrease alignment when emitting
        ...
        Block : Expr ctx ty -> Instr (withLabel ty ctx) ty
        Loop : Expr ctx ty -> Instr (withLabel ty ctx) ty
        If : Expr (withLabel ty ctx) ty -> Expr (withLabel ty ctx) ty -> Instr ctx ty
...
record ModuleDecls where
    constructor MkModuleDecls
    globals : List Global
    functions : List FuncType
    types : List FuncType

record Function (decls : ModuleDecls) where
    constructor MkFunction
    type : Elem ty (types decls)
    -- locals : 
